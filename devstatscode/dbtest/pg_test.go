...
func TestCleanUTF8(t *testing.T) {
	...
	for index, test := range testCases {
		got := devstatscode.CleanUTF8(test.value)
...
func TestTruncToBytes(t *testing.T) {
	...
	// Execute test cases
	for index, test := range testCases {
		gotStr := devstatscode.TruncToBytes(test.value, test.n)
...
func TestTruncStringOrNil(t *testing.T) {
	...
	// Execute test cases
	for index, test := range testCases {
		got := devstatscode.TruncStringOrNil(test.value, test.n)
		if got != test.expected {
			t.Errorf("test number %d, expected %v, got %v", index+1, test.expected, got)
		}
	}
...
func TestBoolOrNil(t *testing.T) {
	result := devstatscode.BoolOrNil(nil)
	if result != nil {
		t.Errorf("test nil case: expected <nil>, got %v", result)
	}
	val := true
	result = devstatscode.BoolOrNil(&val)
	if result != val {
...
func TestNegatedBoolOrNil(t *testing.T) {
	result := devstatscode.NegatedBoolOrNil(nil)
	if result != nil {
		t.Errorf("test nil case: expected <nil>, got %v", result)
	}
	val := true
	result = devstatscode.NegatedBoolOrNil(&val)
	expected := !val
	if result != expected {
...
func TestTimeOrNil(t *testing.T) {
	result := devstatscode.TimeOrNil(nil)
	if result != nil {
		t.Errorf("test nil case: expected <nil>, got %v", result)
	}
	val := time.Now()
	result = devstatscode.TimeOrNil(&val)
	if result != val {
...
func TestIntOrNil(t *testing.T) {
	result := devstatscode.IntOrNil(nil)
	if result != nil {
		t.Errorf("test nil case: expected <nil>, got %v", result)
	}
	val := 2
	result = devstatscode.IntOrNil(&val)
	if result != val {
		t.Errorf("expected %v, got %v", val, result)
	}
}
...
func TestFirstIntOrNil(t *testing.T) {
	...
	// Execute test cases
	for index, test := range testCases {
		got := devstatscode.FirstIntOrNil(test.array)
		if got != test.expected {
			t.Errorf("test number %d, expected %v, got %v", index+1, test.expected, got)
		}
	}
...
func TestStringOrNil(t *testing.T) {
	result := devstatscode.StringOrNil(nil)
	if result != nil {
		t.Errorf("test nil case: expected <nil>, got %v", result)
	}
	val := "hello\x00 world"
	expected := "hello world"
	result = devstatscode.StringOrNil(&val)
	if result != expected {
		t.Errorf("expected %v, got %v", val, result)
	}
}
...
func TestPostgres(t *testing.T) {
	...
	defer func() { devstatscode.FatalOnError(c.Close()) }()

	// Create example table
	devstatscode.ExecSQLWithErr(
		c,
		&ctx,
		devstatscode.CreateTable(
	...
	devstatscode.ExecSQLWithErr(
		c,
		&ctx,
		"insert into test(an_int, a_string, a_dt) "+devstatscode.NValues(3),
		devstatscode.AnyArray{1, "string", time.Now()}...,
	)

	// Get inserted int
	i := 0
	devstatscode.FatalOnError(devstatscode.QueryRowSQL(c, &ctx, "select an_int from test").Scan(&i))
	if i != 1 {
	...
	devstatscode.ExecSQLWithErr(
		...
		&ctx,
		"insert into test(an_int, a_string, a_dt) "+devstatscode.NValues(3),
		devstatscode.AnyArray{11, "another string", time.Now()}...,
	)
	...
	// Insert another row
	devstatscode.ExecSQLTxWithErr(
		tx,
		&ctx,
		"insert into test(an_int, a_string, a_dt) "+devstatscode.NValues(3),
		devstatscode.AnyArray{21, "this will be rolled back", time.Now()}...,
	)

	// Rollback transaction
	devstatscode.FatalOnError(tx.Rollback())
	...
	devstatscode.ExecSQLTxWithErr(
		...
		&ctx,
		"insert into test(an_int, a_string, a_dt) "+devstatscode.NValues(3),
		devstatscode.AnyArray{31, "this will be committed", time.Now()}...,
	)

	// Commit transaction
	devstatscode.FatalOnError(tx.Commit())
	...
	devstatscode.ExecSQLWithErr(
		c,
		&ctx,
		devstatscode.InsertIgnore("into test(an_int, a_string, a_dt) "+devstatscode.NValues(3)),
		devstatscode.AnyArray{1, "conflicting key", time.Now()}...,
	)
...
func getInts(c *sql.DB, ctx *devstatscode.Ctx) []int {
	// Get inserted values
	rows := devstatscode.QuerySQLWithErr(c, ctx, "select an_int from test order by an_int asc")
	defer func() { devstatscode.FatalOnError(rows.Close()) }()
	...
	for rows.Next() {
		devstatscode.FatalOnError(rows.Scan(&i))
		arr = append(arr, i)
	}
