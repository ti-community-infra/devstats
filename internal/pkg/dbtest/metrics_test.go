...
// metricTestCase - used to testlib single metric
...
type metricTestCase struct {
	...
	DebugDB    bool            `yaml:"debug"`  // if set, testlib will not drop database at the end and will return after such testlib, so You can run metric manually via `runq` or directly on DB
...
func TestMetrics(t *testing.T) {
	...
	// We need to know project to testlib
	if ctx.Project == "" {
	...
	// Read per project testlib cases
	...
	if testMetrics != "" {
		...
		for _, m := range ary {
			...
			if !found {
				t.Errorf("no such testlib case '%s'", m)
			}
		}
	}
	...
	for index, test := range testCases {
		...
		if err != nil {
			t.Errorf("testlib number %d (%s): %v", index+1, test.Metric, err.Error())
		}
		if !testlib.CompareSlices2D(test.Expected, got) {
			t.Errorf("testlib number %d (%s), expected:\n%+v\n%+v\ngot testlib case: %+v", index+1, test.Metric, test.Expected, got, test)
		}
		if test.DebugDB {
...
// This prepares raw YAML metric testlib to be executed:
// Binds additional setup function(s)
// if testlib uses "additional_setup_funcs", "additional_setup_args" section(s)
...
// This prepares raw metric testlib to be executed:
// Generates data if testlib uses "data" section
...
// This executes testlib of single metric
// All metric data is defined in "testMetric" argument
// Single metric testlib is dropping & creating database from scratch (to avoid junky database)
// It also creates full DB structure - without indexes - they're not needed in
// small databases - like the ones created by testlib covergae tools
...
// Used when migrating testlib coverage from go source to yaml file
