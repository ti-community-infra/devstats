...
func TestCleanUTF8(t *testing.T) {
	...
	for index, test := range testCases {
		...
		if got != test.expected {
			t.Errorf("testlib number %d, expected %v, got %v", index+1, test.expected, got)
		}
	}
}
...
func TestTruncToBytes(t *testing.T) {
	...
	for index, test := range testCases {
		...
		if gotStr != test.expectedStr {
			t.Errorf("testlib number %d, expected string %v, got %v", index+1, test.expectedStr, gotStr)
		}
		gotLen := len(gotStr)
		if gotLen != test.expectedLen {
			t.Errorf("testlib number %d, expected length %v, got %v", index+1, test.expectedLen, gotLen)
		}
	}
}
...
func TestPostgres(t *testing.T) {
	...
	lib.ExecSQLWithErr(
		...
		&ctx,
		lib.CreateTable(
			"testlib(an_int int, a_string text, a_dt {{ts}}, primary key(an_int))",
		),
	)
	...
	lib.ExecSQLWithErr(
		c,
		&ctx,
		"insert into testlib(an_int, a_string, a_dt) "+lib.NValues(3),
		lib.AnyArray{1, "string", time.Now()}...,
	)

	// Get inserted int
	i := 0
	lib.FatalOnError(lib.QueryRowSQL(c, &ctx, "select an_int from testlib").Scan(&i))
	if i != 1 {
	...
	lib.ExecSQLWithErr(
		c,
		&ctx,
		lib.InsertIgnore("into testlib(an_int, a_string, a_dt) "+lib.NValues(3)),
...
func getInts(c *sql.DB, ctx *lib.Ctx) []int {
	// Get inserted values
	rows := lib.QuerySQLWithErr(c, ctx, "select an_int from testlib order by an_int asc")
