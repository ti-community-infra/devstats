...
func TestIntervalHours(t *testing.T) {
	...
	for index, test := range testCases {
		...
		if got != expected {
			t.Errorf(
				"testlib number %d, expected %v, got %v",
...
func TestComputePeriodAtThisDate(t *testing.T) {
	...
	for index, test := range testCases {
		...
		if got != expected {
			t.Errorf(
				"testlib number %d, expected '%v' from period '%v', hist '%v' for date '%v', got '%v'",
...
func TestDescriblePeriodInHours(t *testing.T) {
	...
	for index, test := range testCases {
		...
		if got != expected {
			t.Errorf(
				"testlib number %d, expected '%v' from %v hours, got '%v'",
...
func TestPeriodParse(t *testing.T) {
	...
	for index, test := range testCases {
		...
		if gotBool != expectedBool {
			t.Errorf(
				"testlib number %d, expected boolean %v, got %v",
				index+1, expectedBool, gotBool,
			)
		}
		if gotDuration != expectedDuration {
			t.Errorf(
				"testlib number %d, expected duration %v, got %v",
...
func TestGetIntervalFunctions(t *testing.T) {
	...
	for index, test := range testCases {
		...
		if gotPeriod != test.expectedPeriod {
			t.Errorf(
				"testlib number %d, expected period %v, got %v",
				index+1, test.expectedPeriod, gotPeriod,
			)
		}
		if gotN != test.expectedN {
			t.Errorf(
				"testlib number %d, expected n %v, got %v",
		...
		if got != expected {
			t.Errorf(
				"testlib number %d, expected start function %v, got %v",
				index+1, expected, got,
			)
		}
		...
		if got != expected {
			t.Errorf(
				"testlib number %d, expected next function %+v, got %+v",
				index+1, expected, got,
			)
		}
		...
		if got != expected {
			t.Errorf(
				"testlib number %d, expected prev function %+v, got %+v",
				index+1, expected, got,
			)
		}
	}
