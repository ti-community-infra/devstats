with commits_data as (
  ...
  where
    ...
    and c.dup_actor_login in (select users_name from tall_users)
  union
  select r.repo_group as repo_group,
    c.sha,
  ...
  where
    ...
    and c.dup_author_login in (select users_name from tall_users)
  union
  select r.repo_group as repo_group,
    c.sha,
  ...
  where
    ...
    and c.dup_committer_login in (select users_name from tall_users)
)
select
