#!/bin/bash
# GRAFANA_DATA=/usr/share/grafana.ob/
...
then
  ...
  do
    ls -l "$f"
    vim --not-a-term -c "%s/'Grafana - '/'OceanBase DevStats - '/g" -c 'wq!' "$f"
  done
  for f in `find ${GRAFANA_DATA} -type f -exec grep -l '"Grafana - "' "{}" \; | sort | uniq`
  do
    ls -l "$f"
