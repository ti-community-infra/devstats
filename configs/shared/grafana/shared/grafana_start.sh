...
MODE=ss FROM='{{grafana_github_allowed_organizations}}' TO="$GF_AUTH_GITHUB_ALLOWED_ORGANIZATIONS" replacer "$cfile" || exit 23
MODE=ss FROM='{{grafana_github_team_ids}}' TO="$GF_AUTH_GITHUB_TEAM_IDS" replacer "$cfile" || exit 24
